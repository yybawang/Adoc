            ...
            <div className={'position-sticky overflow-auto border-right project-left'}>
                <ListGroup>
                    <ListGroup.Item variant={'light'} className={'text-dark'}>
                        <ButtonGroup className={'h4 mb-0 d-inline-block text-truncate'} style={{width: (user.value.id > 0 ? 174 : 254) + 'px'}} title={project.value.name}>{project.value.name}</ButtonGroup>
                        {user.value.id > 0 &&
                        <ButtonGroup>
                            <Link className={'btn btn-link'} to={'/project_manager/' + props.match.params.id} title={'项目设置'}>
                                <Image src={manager} style={{width: '15px'}}/>
                            </Link>
                            <Link className={'btn btn-link'} to={'/post/' + props.match.params.id + '/add'} title={'添加新文档'}>
